...
namespace SURO2
{
    public partial class VisualizacionInternos : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                hfModoInterno.Value = "capturados";
        ...
        private void ConfigurarModo(string modo)
        {
            ...
            int idOrg = Convert.ToInt32(Session["OrgAdscrita"]);

            string busqueda = txtBuscarMultiple.Text.Trim();
            string estatus = ddlEstatus.SelectedValue;

            gvOficios.Visible = true;

            switch (modo)
            {
                case "turnados":
                    lblTitulo.InnerText = "Oficios turnados a mi dirección / departamento / área";
                    gvOficios.DataSource = ObtenerOficiosTurnados(idOrg, busqueda, estatus);
                    gvOficios.DataBind();
                    break;

                case "capturados":
                default:
                    lblTitulo.InnerText = "Mis oficios capturados";
                    gvOficios.DataSource = ObtenerOficiosCapturados(idUsuario, busqueda, estatus);
                    gvOficios.DataBind();
                    break;
            }
        }
        ...
        private DataTable ObtenerOficiosCapturados(int idUsuario, string busqueda, string estatus)
        {
            DataTable dt = new DataTable();
            Funciones funciones = new Funciones();

            using (SqlConnection conn = funciones.ConBD())
            using (SqlCommand cmd = new SqlCommand("SP_MuestraOficiosInternos_xUsuario", conn))
            {
                ...
                cmd.Parameters.AddWithValue("@TipoLista", 1); // 🔹 1 = capturados
                ...
                if (string.IsNullOrWhiteSpace(estatus))
                    ...
                    cmd.Parameters.Add("@Estatus", SqlDbType.NVarChar, 50).Value = estatus;
        ...
        private DataTable ObtenerOficiosTurnados(int idOrg, string busqueda, string estatus)
        {
            DataTable dt = new DataTable();
            Funciones funciones = new Funciones();
...

            {
                ...
                cmd.Parameters.AddWithValue("@idUsuario", Convert.ToInt32(Session["ID"]));
                cmd.Parameters.AddWithValue("@tipoOrg", Convert.ToInt32(Session["TipoOrg"]));
                cmd.Parameters.AddWithValue("@OrgAdscrita", idOrg);
                cmd.Parameters.AddWithValue("@IncluirParaConocimiento", 0);
                cmd.Parameters.AddWithValue("@TipoLista", 2); // 🔹 2 = turnados
        ...
        private void MarcarPestanasModo(string modo)
        {
            string baseClass = "chrome-tab ";

            if (modo == "turnados")
            {
                btnModoCapturados.CssClass = baseClass + "chrome-tab-inactive";
                btnModoTurnados.CssClass = baseClass + "chrome-tab-active";
            }
            else // capturados por defecto
            {
                btnModoCapturados.CssClass = baseClass + "chrome-tab-active";
                btnModoTurnados.CssClass = baseClass + "chrome-tab-inactive";
            }
        }
        ...
        public void CargarOficios()
        {
            ...
            string modo = hfModoInterno.Value;
            ...
            if (tipoLista == 1)
                ...
                lblTitulo.InnerText = "Oficios turnados a mi área";
            ...
            using (SqlCommand cmd = new SqlCommand("SP_MuestraOficiosInternos_xUsuario", conn))
            {
                ...
                cmd.Parameters.Add("@TipoLista", SqlDbType.TinyInt).Value = tipoLista;
                ...
                if (lblEstatus != null)
                {
                    ...
                    if (btnEditar != null)
                    {
                        ...
                        else
                      {
                            ...
                            btnEditar.NavigateUrl = "EditarOficioInterno.aspx?ID=" + id; //mando el ID del oficio a la página de edición en la URL
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = funciones.ConBD())
            using (SqlCommand cmd = new SqlCommand("SP_ObtenerOficioInternoPorID", conn))
            {
        ...
        Funciones func = new Funciones();
            using (SqlConnection con = func.ConBD())
            {
                ...
                SqlCommand cmd = new SqlCommand("SP_MuestraOficioModalInternos", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                if (reader.Read())
                {
                    ...
                    if (HasColumn(reader, "NoOficio"))
                        oficio.NumeroOficio = reader["NoOficio"].ToString();

                    if (HasColumn(reader, "Remitente"))
                        oficio.Remitente = reader["Remitente"].ToString();

                    if (HasColumn(reader, "Telefono") && reader["Telefono"] != DBNull.Value)
                        oficio.Telefono = reader["Telefono"].ToString();

                    if (HasColumn(reader, "Correo") && reader["Correo"] != DBNull.Value)
                        oficio.Correo = reader["Correo"].ToString();

                    if (HasColumn(reader, "TipoDocumento"))
                        oficio.TipoDocumento = reader["TipoDocumento"].ToString();

                    if (HasColumn(reader, "Estatus"))
                        oficio.Estatus = reader["Estatus"].ToString();

                    if (HasColumn(reader, "RutaPDF"))
                        oficio.RutaPDF = reader["RutaPDF"].ToString();

                    if (HasColumn(reader, "Conocimiento") && reader["Conocimiento"] != DBNull.Value)
                        oficio.Conocimiento = Convert.ToBoolean(reader["Conocimiento"]);

                    if (HasColumn(reader, "FechaMaxAtencion") && reader["FechaMaxAtencion"] != DBNull.Value)
                        oficio.FechaMaxAtencion = Convert.ToDateTime(reader["FechaMaxAtencion"]);

                    if (HasColumn(reader, "Asunto"))
                        oficio.Asunto = reader["Asunto"].ToString();

                    if (HasColumn(reader, "FechaOficio") && reader["FechaOficio"] != DBNull.Value)
                        oficio.FechaOficio = Convert.ToDateTime(reader["FechaOficio"]).ToShortDateString();
                }
                ;
                            ...
                            if (allControlsAreAccessible)
                            {
                                ...
                                if (oficio.Estatus == "Turnado") // Si el oficio ya está turnado
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        masterPage.CargarDropdownsInternos(); // Carga el dropdown de direcciones turnadas
                                    }
                                ...
                                if (oficio.Estatus == "Concluido")
                                {
                                    ...
                                    masterPage.MibtnMostrarSeguimiento.Style["display"] = "none";
                                    masterPage.CargarDropdownsInternos();
                                }
...
";
        ...
        protected void btnModoCapturados_Click(object sender, EventArgs e)
        {
            hfModoInterno.Value = "capturados";           
            CargarOficios();
        }

        protected void btnModoTurnados_Click(object sender, EventArgs e)
        {
            hfModoInterno.Value = "turnados";           
            CargarOficios();
        }
