 <%@ Page Title="Default" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="SURO2.Default" %>
<%@ Import Namespace="Newtonsoft.Json" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <style>
        /* ---- Altura uniforme para todas las tarjetas de métricas ---- */
        ...
        /* ---- Chips compactos del semáforo (tarjeta total) ---- */
        ...
        /* ---- Bordes laterales por color para las listas del semáforo ---- */
        ...
        /* ---- Tablas por color: look limpio y acorde ---- */
        ...
        /* Ellipsis en textos largos (No. Oficio / Entidad) */
        .td-ellipsis{
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphContenido" runat="server">
   ...
   <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional" ChildrenAsTriggers="false">
        <ContentTemplate>
            ...
            <div class="container-fluid">
                ...
                <div class="row dashboard-cards">
                    ...
                    <!-- Tarjeta Semáforo (Total) -->
                    <div class="col-xl-2 col-md-4 mb-4">
                        <div class="card border-left-warning shadow h-100 py-2 text-center metric-card semaforo-card">
                            <div class="card-body">
                                <div class="text-xs font-weight-bold text-warning text-uppercase mb-1">
                                   Oficios con Fecha de Atencion.
                                </div>
                                <asp:Label ID="lblSemaforoTotal" runat="server" CssClass="display-4" Text="0"></asp:Label>

                                <div class="chip-row">
                                    <span class="chip rojo">
                                        Vencidos: <span class="value"><asp:Label ID="lblSemaforoRojo" runat="server" Text="0" /></span>
                                    </span>
                                    <span class="chip naranja">
                                        Por vencer: <span class="value"><asp:Label ID="lblSemaforoNaranja" runat="server" Text="0" /></span>
                                    </span>
                                    <span class="chip amarillo">
                                        A Tiempo: <span class="value"><asp:Label ID="lblSemaforoAmarillo" runat="server" Text="0" /></span>
                                    </span>
                                </div>

                                <asp:Label ID="lblSemaforoError" runat="server" Visible="false" 
                                    ForeColor="Red" Style="display:block;margin-top:6px;"></asp:Label>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Gráfica Pie + Barras por Dirección/Departamento/Área -->
                <div class="row">
                ...
                <!-- Listas por color (primer result set del SP) -->
                <div class="row">
                    <!-- ROJO -->
                ...
                <!-- SERIALIZACIÓN DE DATOS PARA JS (se refresca en cada postback parcial) -->
                <script type="text/javascript">
                    var estadisticaData = <%= JsonConvert.SerializeObject(estadisticaList) %>;
                    var semaforoDetalleData = <%= JsonConvert.SerializeObject(semaforoDetalleList) %>;
                </script>
    ...
    <!-- JS para barras horizontales, tablas por color y gráfica PIE -->
    ...
    <script>
        ...
        function getEntidadMapping() {
            ...
            if (tipoOrg === 4) return { label: 'Área', key: 'Quien_Area' };
            if (tipoOrg === 5) return { label: 'Área', key: 'Quien_Area' };
            return { label: 'Dirección', key: 'Quien_Direccion' };
        }
        function setEntidadHeaders() {
            const map = getEntidadMapping();
            document.querySelectorAll('.color-list-card .th-entidad').forEach(th => {
                th.textContent = map.label;
            });
        }

        /* ========= Barras Dir/Dpto/Área ========= */
        ...
        /* ========= Gráfica PIE: crear/actualizar sin fugas ========= */
        var _pieChart = null;
        ...
        function renderOrUpdatePie() {
            ...
            // Si el UpdatePanel reemplazó el canvas, destruimos el chart antiguo
            if (_pieChart) {
                try {
            ...
            if (!_pieChart) {
                // Crear NUEVO chart con tus mismas opciones originales
                _pieChart = new Chart(canvas, {
                    ...
                    options: {
                        ...
                        cutoutPercentage: 80 // igual que tu código original
                    }
                });
            } else {
                // Mismo canvas: solo actualiza datos
        ...
        function initDashboard() {
            ...
            renderBarsWithSelectedEstatus(estadisticaData, "direccionBars", "cardDireccionContainer", "Direccion");
            renderBarsWithSelectedEstatus(estadisticaData, "departamentoBars", "cardDepartamentoContainer", "Departamento");
            renderBarsWithSelectedEstatus(estadisticaData, "areaBars", "cardAreaContainer", "Area");

            renderSemaforoColorTable(semaforoDetalleData, "Rojo", "tblRojoBody", "cardRojo");
            renderSemaforoColorTable(semaforoDetalleData, "Naranja", "tblNaranjaBody", "cardNaranja");
            renderSemaforoColorTable(semaforoDetalleData, "Amarillo", "tblAmarilloBody", "cardAmarillo");
        ...
        // 2) Re-ejecución tras cada postback parcial (UpdatePanel)
